...
#     bash run_bakeoff.sh [DEVICE] [GAMES] [PICKLE_DIR]
...
#   PICKLE_DIR = optional dir of *.pickle real games.  Omitted → SELF-PLAY
#                (uniform-random, generated in RAM, nothing written to disk).
#                Set it to fit on the real GPT-game distribution instead;
#                you only need ONE file (~100k games) to cover 20k+5k.
#
...
if [ -n "${PICKLE_DIR}" ]; then
  ...
  echo "using REAL games from ${PICKLE_DIR}"
else
  echo "using SELF-PLAY games (no --pickle-dir; nothing written to disk)"
fi
