#!/usr/bin/env python
"""
Train a causal transformer on the Bracket Flipping Game.

The model receives the same Othello move sequences as input, but is trained to
predict the state of a "bracket flipping game" — a non-Othello task with
Othello-like flipping mechanics (brackets, color alternation, sequential state
updates) but no board and no spatial logic.

Bracket Flipping Game rules:
  - Each move ID is assigned to one of 5 groups (fixed random table).
  - Odd steps are black (+1), even steps are white (-1).
  - When a move is placed, scan backward for bracket partners (same group,
    same color, with at least one opposite-color move in between).
  - Brackets flip intervening opposite-color moves to the current player's color.
  - State at each step: 60-dim vector with values in {-1, 0, +1}.

The model predicts 3-class state per dimension at each position (cross-entropy).

Run from the project root:

    python -m experiments.mathematical_transformation_experiments.train_bracket_flipper [OPTIONS]
"""
...
STATE_DIM = 60
N_CLASSES = 3  # white (-1), empty (0), black (+1)
N_GROUPS = 5
...
def generate_group_table(seed=42):
    """Generate group assignment table of shape (VOCAB_SIZE,).

    Token 0 (padding) gets group -1. Tokens 1-60 are each assigned to one of
    5 groups (0-4). Each group has ~12 members.
    """
...
def get_group_table(seed=42):
    """Load group table from disk if it exists, otherwise generate and save."""
...
def _compute_single_game(tokens, group_table):
    """Compute bracket flipping game state for a single game.
    ...
    for t in range(T):
        ...
        # Starting color: odd steps (1,3,5,...) = black (+1), even = white (-1)
        current_player = np.int8(1) if (t + 1) % 2 == 1 else np.int8(-1)
        opposite = -current_player
        color[t] = current_player

        group_t = group_table[tok]

        # Scan backward from nearest to farthest
        for j in range(t - 1, -1, -1):
            # Condition 1: same group
            if group_table[tokens[j]] != group_t:
                continue
            # Condition 2: m_j is currently friendly (same as current player's starting color)
            if color[j] != current_player:
                continue
            # Condition 3: at least one opposite-color move between j and t
            has_opposite = False
            ...
            if not has_opposite:
                continue
            # Bracket fires: flip opposite-color moves between j and t
            for k in range(j + 1, t):
                if color[k] == opposite:
                    color[k] = current_player

        # Record state: color[i] for i <= t, 0 for i > t
...
def compute_bracket_states(token_seqs, group_table):
    """Compute bracket flipping game states for a batch of games.
    ...
    Returns:
        states: (N, T, STATE_DIM) int8 array, values in {-1, 0, +1}
    """
    N, T = token_seqs.shape
    states = np.zeros((N, T, STATE_DIM), dtype=np.int8)
    for i in range(N):
        states[i] = _compute_single_game(token_seqs[i], group_table)
    return states
...
class BracketStateDataset(Dataset):
    """Othello token sequences paired with bracket flipping game state targets."""
    ...
    def __init__(self, group_table, max_files=None):
        ...
        for fname in tqdm(files, desc="Loading games"):
            ...
            states = compute_bracket_states(tokens, group_table)
    ...
    def __getitem__(self, idx):
        ...
        # Map {-1, 0, +1} -> {0, 1, 2} for cross-entropy targets
...
class GPTBracketPredictor(GPT):
    ...
    Output: (B, T, STATE_DIM, N_CLASSES) logits.
    Classes: 0 = white (-1), 1 = empty (0), 2 = black (+1).
    """
    ...
    def forward(self, idx, targets=None):
        ...
        logits = self.head(x)  # (B, T, state_dim * n_classes)
...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for x, y in tqdm(loader, desc="  train", leave=False):
        x, y = x.to(device), y.to(device)  # x: (B,T), y: (B,T,60) in {0,1,2}
        logits = model(x)  # (B, T, 60, 3)

        # Mask: non-pad positions expanded to state dim
        non_pad = (x != PAD_IDX)  # (B, T)
        mask = non_pad.unsqueeze(-1).expand_as(y)  # (B, T, 60)

        logits_flat = logits.reshape(-1, N_CLASSES)  # (B*T*60, 3)
        targets_flat = y.reshape(-1)  # (B*T*60,)
        mask_flat = mask.reshape(-1)  # (B*T*60,)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train causal transformer on bracket flipping game"
    )
...
def main():
    ...
    if args.ckpt_dir is None:
        name = f"bracket_pred_gseed{args.group_seed}_{args.layers}L_{args.n_embd}d"
    ...
    if args.eval_only:
        ...
        model = GPTBracketPredictor(config).to(device)
        model.load_state_dict(state_dict)

        print("Loading dataset...")
        dataset = BracketStateDataset(group_table=group_table, max_files=args.max_files)
