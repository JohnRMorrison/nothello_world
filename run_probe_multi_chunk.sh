...
#SBATCH --mem=80GB
#SBATCH --gres=gpu:1
#SBATCH --output=logs/probe_multi_%j.out
...
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python analyze_probe_multi_chunk.py \
    --ckpt "$CKPT" --probe "$PROBE" --hidden 512 \
    --n-chunks 10 --positions-per-chunk 5000
