...
def analyze_one_dataset(results_dir, output_dir, label="OthelloGPT"):
    """Run full analysis on one results directory."""
    samples = load_intervention_data(results_dir)

    # Filter to N=1 interventions for clearest signal
    samples_n1 = [s for s in samples if s["n"] == 1]
    print(f"  N=1 interventions: {len(samples_n1)}")

    if not samples_n1:
        print("  No N=1 samples found, using all")
        samples_n1 = samples

    promo, suppress, ai, al = compute_epsilon_data(samples_n1)
    ...
    plot_epsilon_curves(promo, suppress, ai, al,
                        os.path.join(output_dir, f"epsilon_curves_{label}.png"),
                        label=label)

    delta_legal, delta_illegal = compute_delta_data(samples_n1)
    frac_l, frac_i = plot_delta_curves(
        delta_legal, delta_illegal,
        os.path.join(output_dir, f"delta_curves_{label}.png"),
        label=label)
...
if __name__ == "__main__":
    ...
    # Primary dataset
    print(f"\n=== Analyzing: {args.label} ===")
    result = analyze_one_dataset(args.results_dir, args.output_dir, args.label)
    ...
    for comp in args.compare:
        ...
        r = analyze_one_dataset(comp_dir, args.output_dir, comp_label)
        all_results.append(r)

    # Save summary
    summary_path = os.path.join(args.output_dir, "threshold_analysis_summary.json")
