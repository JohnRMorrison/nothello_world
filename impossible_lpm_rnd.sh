#!/bin/bash
#SBATCH --job-name=imp_rnd
#SBATCH --output=logs/imp_rnd_%A_%a.out
#SBATCH --error=logs/imp_rnd_%A_%a.err
...
python finetune_corruption.py \
    --games-dir ${GAMES_DIR} \
    --output-dir experiments/impossible/losses_lpm_random \
    --label ${GER_LABEL} \
    --random-init \
    --epochs 1 \
    --batch-size 16
